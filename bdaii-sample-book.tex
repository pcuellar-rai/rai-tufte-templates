...
%%
% Book metadata
% \title{Short Title Cool RAI Institute Long Report\thanks{Thanks to Baxter for his inspiration.}}
% \author[R2]{R2D2}
% \publisher{BDAII Press}
...
% For adding subfiles to compiling sequence
\usepackage{subfiles}

% To edit document dimensions
\usepackage{geometry} 

% To place boxes at arbitrary positions
...
% }{Isaac Asimov%, {\itshape Design, Form, and Chaos}
% }
% \vfill
% \openepigraph{%
% ``We are at the point where technology is a part of our lives, and we can no longer afford to think of it as a tool that we can turn on and off.''
% }{Sherry Turkle}
% \vfill
% \openepigraph{%
% \ldots Robots will become the most intelligent creatures on Earth\ldots 
% }{Hans Moravec}
...
% \par\smallcaps{\url{tufte-latex.github.io/tufte-latex/}}
...
and the use of the \doccls{bdaii-book} and \doccls{bdaii-handout} document classes, all built over the original Tufte book and handout classes. .
...
chapters,\sidenote{Parts and chapters are defined for the \texttt{bdaii-book}
...
\end{table}
        ...
        \chapter[On the Use of the bdaii-book Document Class]{On the Use of the \texttt{bdaii-book} Document Class}
\label{ch:bdaii-book}
...
as well.  This can be accomplished using the normal \doccmddef{cite} command.\sidenote{If you use the \texttt{\textbackslash cite} command within
...
\begin{table}[ht]
  ...
  \begin{tabular}{ll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Here are the dimensions of the various margins used in the bdaii-handout class.}
  \label{tab:normaltab}
...
\index{class options|(}
The \doccls{bdaii-book} class is based on the \LaTeX\ \doccls{book}
document class.  Therefore, you can pass any of the typical book
options.  There are a few options that are specific to the
\doccls{bdaii-book} document class, however.
...
\docfilehook{bdaii-common-local.tex}{common}, \docfilehook{bdaii-book-local.tex}{book}, and
\docfilehook{bdaii-handout-local.tex}{handout}.\sloppy

\begin{description}
  \item[\docfilehook{bdaii-common-local.tex}{common}]
  ...
  \item[\docfilehook{bdaii-book-local.tex}{book}] 
  ...
  \item[\docfilehook{bdaii-common-handout.tex}{handout}] 
    If this file exists, it will be loaded after all of the common and
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{lr}
      ...
      Heading level & Value \\
      \midrule
      Part (in \doccls{bdaii-book}) & $-1$ \\
      Part (in \doccls{bdaii-handout}) & $0$ \\
      Chapter (only in \doccls{bdaii-book}) & $0$ \\
      Section & $1$ \\
      Subsection & $2$ \\
      Subsubsection & $3$ \\
      Paragraph & $4$ \\
...
\noindent For example, 
\begin{docspec}
  \doccmdnoindex{documentclass}[symmetric,justified,marginals=raggedouter]\{bdaii-book\}
\end{docspec}
...
\section{Converting from \doccls{article} to \doccls{bdaii-handout}}
...
\section{Converting from \doccls{book} to \doccls{bdaii-book}}
